...
const VALIDATORS = {
  ...
  rfc: {
    test: (v) =>
      !v || /^[A-Z&Ñ]{3,4}\d{6}[A-Z0-9]{2,3}$/.test(v.trim().toUpperCase()),
  ...
  password: {
    test: (v, mode) =>
      mode === "edit" ? !v || v.length >= 6 : v && v.length >= 6,
...
function AutoInput({
  ...
  const handleChange = (e) => {
    ...
    if (v.trim()) {
      ...
      setFiltered(
        options
          .filter((o) => o.valor.toLowerCase().includes(lower))
          .slice(0, 8),
      );
      setOpen(true);
    } else {
...
function PersonsTab({ currentUser, cats, reloadCats }) {
    ...
    if (v.length === 18) {
      ...
      if (parsed) {
        setForm((f) => ({
          ...f,
          curp: v,
          fecha_nacimiento: parsed.date,
          edad: parsed.age,
        }));
      } else {
        setCurpStatus("invalid");
        showMsg("CURP con fecha inválida o fuera de rango.", true);
        return;
      }
  ...
  const handleRFCChange = (val) => {
    ...
    rfcTimer.current = setTimeout(() => {
      const formatErr = validateField("rfc", v, mode);
      if (formatErr) {
        setRfcStatus("invalid");
        return;
      }
      const dup = persons.find(
        (p) => p.rfc && p.rfc.toLowerCase() === v.toLowerCase(),
      );
      if (dup) {
        setRfcStatus("exists");
        showMsg("⚠ Ese RFC ya existe.", true);
  ...
  useEffect(() => {
    ...
    const rfcOk = rfcStatus === "ok";
    if (curpOk && rfcOk) setFieldsUnlocked(true);
    else if (curpStatus === "exists" || rfcStatus === "exists")
      setFieldsUnlocked(false);
  ...
  const handleSave = async () => {
    ...
    if (Object.keys(errs).length > 0) {
      setFormErrors(errs);
      showMsg("Corrija los errores antes de guardar.", true);
      return;
    }
    if (mode === "add" && curpStatus === "exists") {
      showMsg("Esa CURP ya existe.", true);
      return;
    }
    if (mode === "add" && rfcStatus === "exists") {
      showMsg("Ese RFC ya existe.", true);
      return;
    }
    setLoading(true);
    try {
      ...
      showMsg(
        mode === "add" ? "✅ Persona creada." : "✅ Persona actualizada.",
      );
      await fetchPersons();
      await reloadCats();
      handleCancelar();
  ...
  const handleBajas = () => {
    if (!selected) {
      showMsg("Seleccione una persona primero.", true);
      return;
    }
    const p = persons.find((x) => x.curp === selected);
    if (!p) return;
    if (p.cuentas?.length > 0 && p.cuentas[0] !== null) {
      const accs = p.cuentas
        .map((u) => `• ${u.username} (${u.tipo_persona})`)
        .join("\n");
      setModal({
        show: true,
        type: "info",
      ...
      setModal({
        ...
        onConfirm: async () => {
          ...
          setLoading(true);
          try {
            const r = await fetch(`/api/persons/${selected}`, {
              method: "DELETE",
            });
  ...
  const FS = (label, field, options) => (
    <div>
      ...
      <select
        className={`form-select ${!canEdit ? "bg-gray-100 opacity-60" : ""}`}
        value={form[field] || ""}
        disabled={!canEdit}
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex gap-3">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 mb-2">
              <div>
                <label className="form-label">
                  ...
                  {curpStatus === "invalid" && (
                    <span className="text-red-500 text-xs">⚠ inválida</span>
                  )}
                </label>
                <input
                  ...
                  readOnly={curpLocked}
                  onChange={(e) =>
                    !curpLocked && handleCURPChange(e.target.value)
                  }
                />
              </div>
              <div>
                <label className="form-label">
                  RFC *{" "}
                  {!curpLocked && rfcStatus === "ok" && (
                    <span className="text-green-500 text-xs">✓ disponible</span>
                  )}
                  {!curpLocked && rfcStatus === "exists" && (
                    <span className="text-red-500 text-xs">⚠ ya existe</span>
                  )}
                  {!curpLocked && rfcStatus === "invalid" && (
            ...
            {mode === "add" && !fieldsUnlocked && (
              <div className="bg-blue-50 border border-blue-200 text-blue-700 text-sm px-3 py-2 rounded">
                Ingrese una CURP válida (18 caracteres) y un RFC válido que no
                estén registrados para continuar.
              </div>
            )}
            ...
            {(mode !== "add" || fieldsUnlocked) && (
              <div className="grid grid-cols-4 gap-2 mt-2">
                <AutoInput
                  ...
                  options={cats.primer_nombres}
                  onChange={setField("primer_nombre")}
                  disabled={!canEdit}
                  error={formErrors.primer_nombre}
                />
                <AutoInput
                  label="Apellido Paterno"
                  required
                  value={form.apellido_paterno}
                  options={cats.apellidos_paterno}
                  onChange={setField("apellido_paterno")}
                  disabled={!canEdit}
                ...
                <AutoInput
                  ...
                  options={cats.segundo_nombres}
                  onChange={setField("segundo_nombre")}
                  disabled={!canEdit}
                />
                <AutoInput
                  label="Apellido Materno"
                  value={form.apellido_materno}
                  options={cats.apellidos_materno}
                  onChange={setField("apellido_materno")}
                  disabled={!canEdit}
                />
                ...
                <div>
                  ...
                  <input
                    className={`form-input ${!canEdit ? "bg-gray-50 opacity-60" : ""}`}
                    type="date"
                    value={
                      form.fecha_nacimiento
                        ? form.fecha_nacimiento.substring(0, 10)
                        : ""
                    }
                    readOnly={!canEdit}
                    onChange={(e) =>
                ...
                <div>
                  <label
                    className={`form-label ${formErrors.edad ? "text-red-600" : ""}`}
                  >
                    Edad
                  </label>
                  <input
                    className={`form-input bg-gray-50 ${formErrors.edad ? "border-red-400" : ""}`}
                    type="number"
                    value={form.edad || ""}
                    readOnly
                  />
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <input
                    className={`form-input ${!canEdit ? "bg-gray-100 opacity-60 cursor-not-allowed" : ""}`}
                    placeholder="Núm / S/N"
                    value={form.numero || ""}
                    maxLength={20}
                    readOnly={!canEdit}
        ...
        <div className="flex flex-col gap-2 w-36 shrink-0">
          ...
          {mode !== "view" && (
            <div
              className={`text-xs text-center px-2 py-1 rounded font-semibold mt-1
          ...
          <table className="data-table w-full">
            ...
            <tbody>
              ...
              {persons.map((p, i) => (
                <tr
                  key={p.curp}
                  className={selected === p.curp ? "selected" : ""}
...
function UsersTab({ currentUser, cats }) {
  ...
  const handleSave = async () => {
    ...
    if (mode === "add") {
      const dup = users.find(
        (u) =>
          u.curp_persona === form.curp_persona &&
          u.tipo_persona === form.tipo_persona,
      );
      if (dup) {
        showMsg("Esta persona ya tiene una cuenta con ese rol.", true);
        return;
      }
    }
    setLoading(true);
    try {
      ...
      showMsg(
        mode === "add" ? "✅ Usuario creado." : "✅ Usuario actualizado.",
      );
      await fetchUsers();
      handleCancelar();
    } catch {
  ...
  const handleBajas = (u) => {
    if (u.id === currentUser.id) {
      showMsg("No puedes eliminar tu propia cuenta.", true);
      return;
    }
    setModal({
      show: true,
      ...
      <div className="flex gap-3">
        <div className="flex-1 bg-white rounded-lg shadow p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label
                className={`form-label ${formErrors.curp_persona ? "text-red-600" : ""}`}
              >
                Persona *
              </label>
              <select
                ...
                onChange={(e) => {
                  setForm((f) => ({ ...f, curp_persona: e.target.value }));
                  setFormErrors((e2) => ({ ...e2, curp_persona: null }));
                }}
              ...
              >
                ...
                {persons.map((p) => (
                  <option key={p.curp} value={p.curp}>
                    {p.apellido_paterno} {p.apellido_materno || ""},{" "}
                    {p.primer_nombre} — {p.curp}
                  </option>
                ))}
              </select>
              {formErrors.curp_persona && (
                <p className="text-red-500 text-xs mt-0.5">
                  {formErrors.curp_persona}
                </p>
              )}
              {selectedPerson && (
                <div className="mt-1 grid grid-cols-4 gap-2 text-xs text-gray-600 bg-gray-50 rounded p-2">
                  ...
                  <span>
                    🎂 Edad: <strong>{selectedPerson.edad || "—"}</strong>
                  </span>
                  <span>
                    🪪 RFC: <strong>{selectedPerson.rfc || "—"}</strong>
                  </span>
                  <span>
                    👤 Género: <strong>{selectedPerson.genero || "—"}</strong>
                  </span>
                </div>
              )}
            </div>
            ...
            <div>
              <label
                className={`form-label ${formErrors.tipo_persona ? "text-red-600" : ""}`}
              >
                Tipo *
              </label>
              <select
                ...
                onChange={(e) => {
                  setForm((f) => ({ ...f, tipo_persona: e.target.value }));
                  setFormErrors((e2) => ({ ...e2, tipo_persona: null }));
                }}
                disabled={mode === "view" || mode === "edit"}
              >
            ...
            <div>
              <label
                className={`form-label ${formErrors.username ? "text-red-600" : ""}`}
              >
                Usuario *
              </label>
              <input
            ...
            <div>
              <label
                className={`form-label ${formErrors.password ? "text-red-600" : ""}`}
              >
                Contraseña
                {mode === "edit" && (
            ...
            <tbody>
              ...
              {users.map((u, i) => (
                ...
                >
                  ...
                  <td>
                    <span
                      className={`text-xs px-2 py-0.5 rounded font-semibold ${
                        u.tipo_persona === "ADMIN"
                          ? "bg-purple-100 text-purple-800"
                          : u.tipo_persona === "TEACHER"
                            ? "bg-blue-100 text-blue-800"
                            : u.tipo_persona === "STUDENT"
                              ? "bg-green-100 text-green-800"
                              : u.tipo_persona === "TUTOR"
                  ...
                  <td>
                    <span
                      className={`text-xs px-1.5 py-0.5 rounded ${u.activo ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                    >
...
export default function AdminPage() {
  ...
  return (
    <>
      <Head>
        <title>Sistema Escolar – Administración</title>
      </Head>
      <div className="min-h-screen flex flex-col bg-gray-100">
        <header
          style={{ background: "var(--navy)" }}
          className="text-white px-4 py-2 flex items-center gap-4 shadow"
        >
        ...
        <main className="flex-1 p-4 overflow-auto">
          ...
          {tab === "users" && (
            <UsersTab currentUser={currentUser} cats={cats} />
          )}
        </main>
